...
pair<int,int> deg_sol(vector<double>& ratio, double minsol,vector<int>&basv,vector<double>& key, vector<vector<double>>& A);
...
void simplex(vector<vector<double>>& A, vector<double>& B, vector<double>& C,vector<int>& basis) 
{
    ...
    while (check)
    {
        ...
        if(degen){
            ...
            return;
            auto it = deg_sol(ratio,mininsol,basv,keycolval,A);
            keyrow=it.first;
            mininsol=it.second;
        ...
        for (int i = 0; i < m; i++)
        {
            cout << "x"<<basv[i]+1<<setw(8)<<cb[basv[i]]<<setw(8);
            cout << sol[i] << setw(8);
            for (int j = 0; j < (n); j++)
            {
                cout << A[i][j] << setw(8);
...
pair<int,int> deg_sol(vector<double>& ratio, double minsol,vector<int>&basv,vector<double>& key, vector<vector<double>>& A){
    ...
    set<int> idx1;
    for(auto &x:basv)idx1.insert(x);
    int ind=-1;
    for(int i=0;i<A[0].size();i++){
        double mn = INT_MAX;
        int cnt=0;
        if(idx1.find(i)==idx1.end())continue;
        for(int j=0;j<A.size();j++){
            if(idxs.find(j)==idxs.end())continue;
            if(A[j][i]<0)continue;
            double temp = A[j][i]/key[j];
            cout<<temp<<" ";
            if(temp<mn){
                mn=temp;
                ind=j;
...
void printRes(vector<vector<double>>& A, vector<double>& b, vector<double>& c,  vector<vector<string>>& var, vector<int>&rstrict,vector<int>& basis) {
    ...
    //     cout<<0<<(i < b.size()-1?", ":")");
    // }
    // vector<int> cb(b.size(),0);
    // cout<<"^T"<<endl;
    // cout<<endl;
    // auto xb = b;
    // vector<double> del(mx,0);
    // for(int i=0;i<orig_func.size();i++)del[i]=xb[i]*cb[i] - orig_func[i];

    // for(int i=0;i<mx;i++){
    //     cout<<"del_"<<i+1<<"="<<del[i]<<(i<mx?", ":"");
    // }
